...
export default Ember.Service.extend({
        ...
        modulos.forEach(mod => {
          ...
          if (mod.get('atividades').get('length') > 0) {
            mod.get('atividades').forEach(ativ => {
              ...
              obj_ativ.secoes.sort(function(a, b){return a.idx-b.idx});
  ...
  async calculateProgress(pessoa, modulos) {
    ...
    let estadosQuestoes = await pessoa.get('estadoQuestoes');
    ...
    if (modulos) {
      ...
      modulos.forEach(async mod => {
        let modId = mod.get('id');
        if (matriculas) {
          let matModId;
          progressoRealValue = '0% ';
          matriculas.forEach(mat => {
                  ...
                  if (cont.get('quiz').get('questoes')) {
                    ...
                    cont.get('quiz').get('questoes').forEach(questao => {
                      ...
                      questao.get('alternativas').forEach(alternativa => {
                        ...
                        if (resposta) {
                          estadosQuestoes.forEach(function(estado){
                            if (estado.get('questao').get('id') == obj_questao.id){
                              obj_questao['tempoquestaoalias'] = estado.get('tempoQuestao') + ' segundos';
                            }
                          })
                          obj_questao['respostas'].push({
                            'id': alternativa.get('id')
