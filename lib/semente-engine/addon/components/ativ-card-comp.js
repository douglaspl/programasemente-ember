...
export default Component.extend({
    ...
    tagName: 'li',
    classNameBindings: ['isDisabled:class--is-disabled', 'isCompleted:class--is-completed'],
    attributeBindings: ['style'],
    style: Ember.computed('atividade', function() {
        return this.get('atividade.backgroundImage');
    }),
    appstate: Ember.inject.service(),
    isCompleted: Ember.computed('value', function() {
        if (this.get('value') === 100) return true;
        else return false;
    }),
    isDisabled: Ember.computed('value', function() {
        if (this.get('value') === 0) {
            if (this.get('aula_liberada'))
            {
                return false;
            }
            return true;
