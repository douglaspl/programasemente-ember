...
export default Ember.Component.extend({
    ...
    tnName: Ember.computed('', function () {
        if (this.get('conteudo.thumbnail') && this.get('conteudo.thumbnail').includes("Thumbnail_")){
            let thumbs = this.get('conteudo.thumbnail').split("/");
            let thumb = thumbs[thumbs.length - 1];
            let index = thumb.indexOf("Thumbnail_");
            let s = thumb.substring(index + 10);
            return s
        } else return this.get('conteudo.thumbnail')
    }),
    actions: {
