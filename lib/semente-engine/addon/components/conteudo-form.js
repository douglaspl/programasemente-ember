...
=======
              imageData = e.target.result;
            //   debugger;
              if (file.type.includes('jpeg') ||file.type.includes('png')) conteudo.set('tipo', 'Imagem');
              else if (file.type.includes('pdf')) conteudo.set('tipo', 'Documento');
              conteudo.set('filename', file.name);
              conteudo.set('path', imageData);
            };

>>>>>>> c489281ec7b6f4da28d1bc92c9390fe0eea789d6
            if (file) {
                reader.readAsDataURL(file); //pdf to base64
            }
        },
