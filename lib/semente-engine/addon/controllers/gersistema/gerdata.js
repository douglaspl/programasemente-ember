...
export default Controller.extend({
  ...
  queryParams: ["page", "perPage", "instituicao_id", "ordem", "str_search", "dummy"],
  ...
  sendFiletoServer(file) {
    ...
    let temp = 'Bearer ';
    let userToken = temp.concat(tok);
    let final_url = this.get('envnmt.host') + '/' + this.get('envnmt.namespace') + '/pessoas/cadastrolote';
    return new Ember.RSVP.Promise(function (resolve, reject) {
      let xhr = new XMLHttpRequest();
  ...
  actions: {
    ...
    userActionDisplay(param) {
      ...
      let classToOpen = 'submenu--each-animation-is-show';
    ...
    listItemCheck(param) {
      let actionClass = document.getElementsByClassName('j-user-action');
      for (let i = 0; i < actionClass.length; i++) {
        actionClass[i].classList.remove('submenu--each-animation-is-show');
      }
    ...
    userModal(param) {
      ...
      if (param) {
        ...
        this.set('new_user_area', pessoa.get('area.id'));
      ...
      // new user
        ...
        let role = document.getElementById('new_user_role');
        role.options[0].selected = true;
        let area = document.getElementById('new_user_area');
    ...
    userSave(param) {
      ...
      let area = document.getElementById('new_user_area').value;
      ...
      // save edited user
      if (param) {
        let pessoa = this.get('pessoa_selected_edit');
        ...
        if (area) {
          pessoa.set('area', this.get('store').peekRecord('area', area));
        }
        let old_modulos = [];
        pessoa.get('modulos').forEach((mod) => {
          old_modulos.pushObject(mod);
        });
        let modulos = this.get('modulos');
        modulos.forEach((mod) => {
          pessoa.get('modulos').removeObject(mod);
          if (document.getElementById('ckb_new_p_' + mod.id).checked) {
            pessoa.get('modulos').pushObject(mod);
          }
        });
        userToSave = pessoa;
      }
      // save new user
        ...
        if (area) {
          pessoa['area'] = this.get('store').peekRecord('area', area);
        }
