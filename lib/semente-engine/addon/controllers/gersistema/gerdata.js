...
export default Controller.extend({
  ...
  queryParams: ["page", "perPage", "instituicao_id", "ordem", "str_search", "dummy", "ano1", "ano2", "ano3","pre","tur"],
  ...
  actions: {
    ...
    filterTurma(){
      ...
      if (turma == 0){
        ...
        let mod = this.get('store').peekRecord('turma',turma).get('modulo');
    ...
    userSave(param) {
      ...
      let genero, matricula, nascimento, ano, turma;
      if (role == 'aluno'){
        ...
        ano = document.getElementById('new_user_ano').value;
        turma = document.getElementById('new_user_turma').value;
      ...
      if (param) {
        ...
        if (role == 'aluno') {
          ...
          if (pessoa.get('nascimento') != nascimento){
            pessoa.set('nascimento', nascimento)
          }
          if (pessoa.get('ano') != ano){
            pessoa.set('ano', ano);
          }
          if (pessoa.get('turma') != turma){
            pessoa.set('turma', turma)
          }
        }
        ...
        if (role == 'aluno' || role == 'instrutor'){
          ...
          turmas.forEach(function(tur){
            ...
            if (document.getElementById('ckb_new_p_' + tur.id).checked){
              pessoa.get('turmas').pushObject(tur);
