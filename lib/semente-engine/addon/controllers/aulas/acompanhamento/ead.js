...
export default Controller.extend({
  ...
  actions: {
    ...
    filterano(ano) {
      ...
      if (currentAnos) {
        if (currentAnos.includes(ano + ',')) {
          ...
          this.send('atualizaTurmasRemove', ano);
          ...
          currentAnos += ',' + ano;
          this.send('atualizaTurmasAdd', ano);
        }
    ...
    atualizaTurmasAdd(ano){
      let inst_filtered = this.get('inst_filtered');
      if (inst_filtered){
        let turmas = inst_filtered.get('turmas');
        turmas.forEach(function(t){
          if (t.get('modulo').get('id') == ano){
            document.getElementById('opt_tur_' + t.id).style.display = 'block';
          }
        })
    ...
    atualizaTurmasRemove(ano){
      let inst_filtered = this.get('inst_filtered');
      if (inst_filtered){
        let turmas = inst_filtered.get('turmas');
        turmas.forEach(function(t){
          if (t.get('modulo').get('id') == ano){
            document.getElementById('opt_tur_' + t.id).style.display = 'none';
          }
        })
      }else if (!this.get('hasChildren')){
        let turmas = this.get('inst_selected').get('turmas');
