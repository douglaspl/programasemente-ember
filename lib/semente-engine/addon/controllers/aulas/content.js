...
export default Controller.extend({
  ...
  UltimaAplicacao: Ember.computed('model', function(){
    ...
    .filterBy('aplicado', true)
    .filterBy('turma.instituicao.id', this.get('pessoa.instituicao.id'));
    let ultimaData = aplicacoes.sortBy('dataAplicacao').reverse()[0];
    return ultimaData;
  }),
  ...
  actions: {
    ...
    refreshSelectedAula(selectedAulaId) {
      ...
      let ultimaAplicacao =  aula.get('aplicacoes').filterBy('aplicado', true).filterBy('turma.instituicao.id', this.get('pessoa.instituicao.id')).sortBy('dataAplicacao').reverse()[0];
