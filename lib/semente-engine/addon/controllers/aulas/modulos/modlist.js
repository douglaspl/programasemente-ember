...
export default Controller.extend({
  ...
  sortingKey: ['idx'],
  activeAula: true,
  parentController: Ember.inject.controller('aulas.modulos'),
  aulasController: Ember.inject.controller('aulas'),
  sortedAtividades: Ember.computed.sort('model.atividades', 'sortingKey'),
  livros: Ember.computed('parentController.model', function(){
    return this.get('parentController.model.plataformaAnos').mapBy('livros').get('firstObject')
...
// ########## Douglas - Lógica para scroll da timeline [Fim]
  ...
  actions: {
    ...
    goToModulos(modIdx) {
      this.transitionTo('aulas.modulos.modlist', modIdx);
    },
    goAtividade() {
      // curso selecionado
      ...
      if (course) {
        ...
        if (courseProgress == 0 || courseProgress == 100) {
          this.transitionToRoute('aulas.modulos.modetails.ativdetails.secdetails', this.get('model'),
            courseClasses[0].id,
