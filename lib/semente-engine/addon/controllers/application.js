...
export default Ember.Controller.extend({
  ...
  envnmt: ENV.APP,
  filesList: [],
  modulo: Ember.computed('data','appstate.upState', function() {
  ...
  instLibVerify() {
    ...
    let temp = 'Bearer ';
    let userToken = temp.concat(tok);
    let instEndpoint = this.get('env.host') + '/' + this.get('env.namespace') + '/pessoas?include=instituicao';
    let that = this;
    ...
    if (obj) {
      ...
      Ember.run.schedule('afterRender', function () {
        ...
        if ((obj.role == 'admin') && (window.location.pathname.search('styleguide') > 1)) that.transitionToRoute('styleguide') 
         else if (obj.role == 'aluno') that.transitionToRoute('modulos.modlist', 1)
      ...
      if (inst_array) {
        if (inst_array.length === 1) {
          ...
          Ember.run.schedule('afterRender', function () {
            ...
            //debugger;
            if (pessoa.attributes.role == 'aluno') that.transitionToRoute('modulos.modlist', 1);
            else that.transitionToRoute('administracao');
            // #########################################
            // #########################################
            // #########################################
    ...
    selectPessoa: function (param) {
      ...
      data.forEach((pessoa) => {
        if (pessoa.id === param) {
          ...
          //debugger;
          if (pessoa.attributes.role == 'aluno') this.transitionToRoute('modulos.modlist', 1);
