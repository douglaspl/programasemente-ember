...
export default Ember.Controller.extend({
  ...
  getModulo(){
    ...
    let instEndpoint = this.get('env.host') + '/' + this.get('env.namespace') + '/pessoas/'+ localData.id +'?include=modulos';
    let that = this;
      ...
      function handler() {
        ...
        if (this.readyState === this.DONE) {
          if (this.status === 200 || this.status === 204) {
              debugger;
            	if (this.response.included.length==1){
                let modulo = this.response.included[0].attributes;
                that.set("ModuloIdx",modulo.idx);
                that.set("ModuloName",modulo.name);
                if (modulo.idx==1) that.set("primeiroAno", true);
                else if (modulo.idx==2) that.set("segundoAno", true);
                else if (modulo.idx==3) that.set("terceiroAno", true);
                let moduloPage = {
                  "id": modulo.idx,
                  "name": modulo.name,
                };
                localStorage.setItem('modulo-page', JSON.stringify(moduloPage));
                return modulo;
              }else{
                this.response.included.forEach(function(modulo){
                  debugger;
                  let idx = modulo.attributes.idx;
                  if (idx==1) that.set("primeiroAno", true);
                  else if (idx==2) that.set("segundoAno", true);
                  else if (idx==3) that.set("terceiroAno", true);
                });
              }
  ...
  //         if (this.status === 200 || this.status === 204) {
  //             this.response.included.forEach(function(modulo){
  //               let atributos = modulo.attributes;
  ...
  oneInst: Ember.computed('model', function () {
    ...
    if (obj) {
      ...
      Ember.run.schedule('afterRender', function () {
        ...
        let modulo = JSON.parse(localStorage.getItem('modulo-page'));
        if ((obj.role == 'admin') && (window.location.pathname.search('styleguide') > 1)) that.transitionToRoute('styleguide') 
         else if (obj.role == 'aluno') that.transitionToRoute('modulos.modlist', modulo.id)
        else that.transitionToRoute('administracao');
      ...
      if (inst_array) {
        if (inst_array.length === 1) {
          ...
          Ember.run.schedule('afterRender', function () {
            ...
            else that.set('id_instituicao', pessoa.instituicao_id);
            ...
            if (pessoa.attributes.role == 'aluno') {
              ...
              let moduloid = that.get('model')[0].relationships.modulos.data[0].id;
              that.transitionToRoute('modulos.modlist', moduloid);
            ...
            // #########################################
            // #########################################
            //that.transitionToRoute('modulos');
            that.libSet();
          });
          return true;
        } else return false;
  ...
  setTitle: function () {
    ...
    // follow up module
      ...
      followUp.firstChild.classList.add("global-nav__link--is-active");
    ...
    // adminsitration module
      ...
      administration.firstChild.classList.add("global-nav__link--is-active");
    ...
    selectPessoa: function (param) {
      ...
      data.forEach((pessoa) => {
        if (pessoa.id === param) {
          ...
          let modulo = JSON.parse(localStorage.getItem('modulo-page'));
          if (pessoa.attributes.role == 'aluno') this.transitionToRoute('modulos.modlist', modulo.id);
          else this.transitionToRoute('administracao');
          // #########################################
          // #########################################
          // #########################################
