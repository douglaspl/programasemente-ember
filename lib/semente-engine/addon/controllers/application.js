...
export default Ember.Controller.extend({
  ...
  ModuloIdx: Ember.computed('model', function () {
    function findMatricula(element,index,array){
    ...
    let moduloid = matricula.attributes["modulo-id"];

    return moduloid;
  }),
  InstituicaoId: Ember.computed('model', function () {
    function findInstituicao(element,index,array){
  ...
  primeiroAno: Ember.computed('model', function () {
    if(this.get('model').included.find(function(obj){return obj.type=='matricula' && obj.attributes['modulo-id']==1})) return true;

  }),
  segundoAno: Ember.computed('model', function () {

    if(this.get('model').included.find(function(obj){return obj.type=='matricula' && obj.attributes['modulo-id']==2})) return true;
    
  }),
  terceiroAno: Ember.computed('model', function () {

    if(this.get('model').included.find(function(obj){return obj.type=='matricula' && obj.attributes['modulo-id']==3})) return true;
    
  }),
  preVestibular: Ember.computed('model', function () {

    if(this.get('model').included.find(function(obj){return obj.type=='matricula' && obj.attributes['modulo-id']==4})) return true;
    ...
    if (obj) {
      ...
      Ember.run.schedule('afterRender', function () {
        ...
        if ((obj.role == 'admin') && (window.location.pathname.search('styleguide') > 1)) that.transitionToRoute('styleguide') 
         else if (obj.role == 'aluno') that.transitionToRoute('modulos.modlist', that.get('ModuloIdx'))
    ...
    selectPessoa: function (param) {
      ...
      data.forEach((pessoa) => {
        if (pessoa.id === param) {
          ...
          if (pessoa.attributes.role == 'aluno') this.transitionToRoute('modulos.modlist', that.get('ModuloIdx'));
