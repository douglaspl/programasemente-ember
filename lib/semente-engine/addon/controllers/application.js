...
export default Ember.Controller.extend({
  ...
  hasReport: Ember.computed('model', function(){
    ...
    if (obj) {
      obj = JSON.parse(obj);
      if (obj.role == 'aluno' || obj.role == 'responsavel') {
        return false;
      } else {
        return true;
      }
  ...
  hasManagement: Ember.computed('model', function(){
    // debugger;
    let obj = localStorage.getItem('person_logged');
    if (obj) {
      obj = JSON.parse(obj);
      if (obj.role == 'diretor' || obj.role == 'gestor' || obj.role == 'admin') {
        return true;
      } else {
        return false;
      }
  ...
  oneInst: Ember.computed('model', function () {
    ...
    let pessoa = this.get('store').peekRecord('pessoa', person_read.id);
    let that = this;
    if (obj) {
      obj = JSON.parse(obj);
      Ember.run.schedule('afterRender', function () {
        ...
        else if (obj.role == 'aluno' || obj.role == "responsavel" || obj.role == "instrutor"){
          if (that.get('hasModulo')){
            that.transitionToRoute('modulos.modlist', that.get('ModuloIdx'));
          } else {
            if (pessoa) {
              if (pessoa.get('role') == 'instrutor' && !pessoa.get('isAplicador')) {
                that.transitionToRoute('aulas.bibliotecaindex');
              }
            }
      ...
      if (inst_array) {
        if (inst_array.length === 1) {
          ...
          Ember.run.schedule('afterRender', function () {
            ...
            else if (pessoa.attributes.role == 'instrutor') {
              if (that.get('hasModulo')){
                that.transitionToRoute('modulos.modlist', that.get('ModuloIdx'));
              } else {
                if (pessoa) {
            ...
            else if (pessoa.attributes.role == 'responsavel') {
              that.transitionToRoute('aulas');
            }
            else that.transitionToRoute('administracao');
            // #########################################
