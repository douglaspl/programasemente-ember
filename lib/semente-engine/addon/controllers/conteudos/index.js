...
export default Controller.extend({
  ...
  conteudos: Ember.computed('model', function(){
    let conts = this.get('model.platconteudos').filter(c => this.get('selectedPublicos').mapBy('name').every(sp => c.get('publicos').mapBy('name').includes(sp)));
  ...
  temas: Ember.computed('model', function(){
    return this.get('store').peekAll('tema')   
  }),
