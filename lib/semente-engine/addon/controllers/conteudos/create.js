  ...
  validateForm: function() {
    ...
    // Validação da capa
        if (conteudoAtual.get('tipo') == 'Documento' && conteudoAtual.get('thumbnail') == null) {
          this.set('coverValidation', true);
          error_publico.innerText = 'Você precisa especificar uma capa quando o conteúdo é um documento';
          errors.push('capa');
        } else {
          this.set('coverValidation', false);
          errors = errors.filter(function(item) {
            return item !== "capa"
          })
        }  
      ...
      // Validação aulas
        ...
        if (agrupamentoAtual.id != 3 && conteudoAtual.get('aulas').get('length') < 1) {
          this.set('classValidation', true);
          error_publico.innerText = 'Selecione pelo menos um segmento, ano/série e bloco/aula para este conteúdo';
          errors.push('aulas');
        } else {
        ...
        if (this.get('classValidation') || this.get('pubValidation') || this.get('coverValidation')) {
