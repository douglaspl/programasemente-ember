...
export default Controller.extend({
  ...
  getOut() {
    ...
    let proximaSecao = atividade.get('secoes').filterBy('idx', idx + 1).get('firstObject');
    ...
    let proximaQuiz = proximaSecao.get('conteudos').get('firstObject').get('quiz').get('questoes');
    ...
    if (path.search('transition') > 1) {
      ...
      if (ultimaObrigatoria) {
        ...
        if (ultimaDaAula) {
          ...
          atividade.get('modulo').get('atividades').filterBy('idx', this.get('model').get('atividade').get('idx') + 1).get('firstObject').get('secoes').get('firstObject').get('id')
          );
        } else {
  ...
  proximaSecao: Ember.observer("model", "appController.transited", function () {
    ...
    if (next_atividade) {
      ...
      let next_ativsec_id = next_atividade.get('secoes').get('firstObject').get('id');
