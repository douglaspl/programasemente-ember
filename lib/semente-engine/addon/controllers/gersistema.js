...
export default Controller.extend({
  ...
  modulos_list: Ember.computed('model',function(){
    // let listaModulos = [];
    // this.get('model').modulos.forEach(function(modulo){
    //   let modlist = {
    //         'id': modulo.id,
    //         'name': modulo.data.name,
    //         'coverImg': modulo.data.coverImage,
    //     }
    //     modlist.listaTurmas = [];
    //     modulo.get('turmas').forEach(function(tur){
    //       modlist.listaTurmas.push(tur)
    //     })
    //   listaModulos.push(modlist);
    // })
    // debugger;
  ...
  actions: {
    ...
    checkModulo(id){
      let modulosSelected_list = [];
      this.get('model').data[0].relationships.modulos.data.forEach(function(modulo){
        modulosSelected_list.push(modulo.id);
      });
      return modulosSelected_list.includes(id);
    },
    ...
    filterInst() {
      ...
      if (matchValue.value) {
        let searchResult = instList.filter(function (i) {
          if ((i.attributes.name).toLowerCase().match(new RegExp((matchValue.value).toLowerCase(), 'g'))) {
            return i;
          }
        });
