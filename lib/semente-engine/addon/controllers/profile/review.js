...
export default Controller.extend({
  ...
  validateFormFunction: function () {
    ...
    formInputs.forEach(input => {
      ...
      if (input.dataset.required == "true" && input.dataset.type == "e-mail" && input.parentElement.nextElementSibling.classList.contains("alert--is-show")) {
        naoValidados.push(input);
      };
    ...
    formInputs.forEach(i => {
      ...
      i.parentElement.nextElementSibling.classList.remove("alert--is-show");
    });
    ...
    if (naoValidados.length > 0) {
      ...
      naoValidados.forEach(i => {
        if (!i.type == "checkbox") {
          ...
          i.parentElement.nextElementSibling.classList.add("alert--is-show");
        }
      });
  ...
  actions: {
    ...
    carouselFoward: function () {
      let errors = document.querySelectorAll(".alert--is-show");
      errors.forEach(error => {
        error.classList.remove('alert--is-show');
      })
    ...
    refreshCel(celular) {
      let value = celular
      let responsavel = this.get('responsavel');
      responsavel.set('telefone', value);
    ...
    celReachedMaxLength() {
      ...
      if (target.value.length == 15) {
        target.parentElement.nextElementSibling.classList.remove("alert--is-show");
      }
    },
    ...
    trimAll() {
      ...
      inputs.forEach(input => {
        ...
        input.value = trimmed;
        input.parentElement.nextElementSibling.classList.remove("alert--is-show");
      })
